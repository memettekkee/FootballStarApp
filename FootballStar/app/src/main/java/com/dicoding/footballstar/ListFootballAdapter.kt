...
class ListFootballAdapter(private val listFootball: ArrayList<Football>) : RecyclerView.Adapter<ListFootballAdapter.ListViewHolder>() {
    ...
    override fun getItemCount(): Int = listFootball.size

    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        val (name, description, photo) = listFootball[position]
        holder.imgPhoto.setImageResource(photo)
        holder.tvName.text = name
        holder.tvDescription.text = description

        holder.itemView.setOnClickListener { onItemClickCallback.onItemClicked(listFootball[holder.adapterPosition])}
    }
    ...
    interface OnItemClickCallback {
        fun onItemClicked(data: Football)
    }
}